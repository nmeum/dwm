...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(stext) - lrpad / 2; /* no right padding so status text hugs the corner */
...
tile(Monitor *m)
{
	...
	if (n == 0)
		return;
	mgappx = (n == 1) ? 0 : gappx; /* no gaps if there is just one client */

	if (n > m->nmaster)
		mw = m->nmaster ? m->ww * m->mfact : 0;
	else
		mw = m->ww - mgappx;
	for (i = 0, my = ty = mgappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i) - mgappx;
			resize(c, m->wx + mgappx, m->wy + my, mw - (2*c->bw) - mgappx, h - (2*c->bw), 0);
			if (my + HEIGHT(c) + mgappx < m->wh)
				my += HEIGHT(c) + mgappx;
		} else {
			h = (m->wh - ty) / (n - i) - mgappx;
			resize(c, m->wx + mw + mgappx, m->wy + ty, m->ww - mw - (2*c->bw) - 2*mgappx, h - (2*c->bw), 0);
			if (ty + HEIGHT(c) + mgappx < m->wh)
				ty += HEIGHT(c) + mgappx;
		}
}
